...
if($sellerAddr)
{
	...
	if ($sellerAddr)
	{
		list($string, $text) = $pdf->splitString(CSalePdf::prepareToPdf($sellerAddr), $width - 10);
		$pdf->Cell($width, 15, $string, 0, 0, 'R');
		$pdf->Ln();
	}
}
...
if($sellerPhone)
{
	...
	$pdf->Cell($width, 15, CSalePdf::prepareToPdf($sellerPhone), 0, 0, 'R');
	$pdf->Ln();
}
...
if($sellerEmail)
{
	...
	$pdf->Cell($width, 15, CSalePdf::prepareToPdf($sellerEmail), 0, 0, 'R');
	$pdf->Ln();
}
